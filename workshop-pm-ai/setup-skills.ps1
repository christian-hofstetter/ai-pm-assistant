h# PowerShell script to install PM Skills into .kiro/skills/
...
if (-not (Test-Path $PmSkillsDir)) {
    ...
    Write-Host ""
    Write-Host "First clone it:"
    Write-Host "  git clone https://github.com/phuryn/pm-skills.git"
    Write-Host ""
    Write-Host "Then run:"
    Write-Host "  .\setup-skills.ps1 -PmSkillsDir ..\pm-skills"
    exit 1
}
...
if ($copied -gt 0) {
    ...
    Write-Host "You can now open this folder in Kiro IDE."
} else {
